...
team_name_map = {
    ...
    'Oklahoma City Thunder': '俄克拉荷馬城雷霆',
    'Orlando Magic': '奧蘭多魔術',
...
def createTodaysGames(games, df, odds):
    ...
    for game in games:
        ...
        last_home_date = \
        home_games.loc[schedule_df['Date'] <= datetime.today()].sort_values('Date', ascending=False).head(1)[
            'Date'].iloc[0]
        last_away_date = \
        away_games.loc[schedule_df['Date'] <= datetime.today()].sort_values('Date', ascending=False).head(1)[
            'Date'].iloc[0]
...
def main():
    ...
    if args.A:
        print("---------------XGBoost Model Predictions---------------")
        xgb = XGBoost_Runner.xgb_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds)
        print("-------------------------------------------------------")
        data = tf.keras.utils.normalize(data, axis=1)
        print("------------Neural Network Model Predictions-----------")
        nn = NN_Runner.nn_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds)
        print("-------------------------------------------------------")
        print("XGB")
        for game in xgb["predictions"]:
            home_team = team_name_map[game['home_team']]
            away_team = team_name_map[game['away_team']]
            if game['winner'] == 'home':
                print(
                    f"場次：[{home_team}] vs [{away_team}] => 預測：{home_team} {'小分' if game['over_under'] == 'under' else '大分'} {game['odds']} ")
            else:
                print(
                    f"場次：[{home_team}] vs [{away_team}] => 預測：{away_team} {'小分' if game['over_under'] == 'under' else '大分'} {game['odds']} ")
        print("NN")
        for game in nn["predictions"]:
            home_team = team_name_map[game['home_team']]
            away_team = team_name_map[game['away_team']]
            if game['winner'] == 'home':
                print(
